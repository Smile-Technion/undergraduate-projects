...
class ProjectEnvImp(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def step(self, a):
        if (self.sim.data.time == 0):
            ...
            if(self.sim.data.time==self.dt):
                ...
                #if((impedance_norm_xy > 0.2) or (impedance_norm_phi > 0.2)):
                if(impedance_norm_x > 0.05):
                    reward_impedance_xm_x = -10
                if(impedance_norm_y > 0.05):
                    reward_impedance_xm_y = -10
                if(impedance_norm_phi > 0.05):
                    reward_impedance_xm_phi = -10

                #if (((impedance_norm_xy < 0.2) and (impedance_norm_xy > 0.01)) or ((impedance_norm_phi < 0.2) and (impedance_norm_phi  > 0.01))):
                if ((impedance_norm_x < 0.05) and (impedance_norm_x > 0.01)):
                    reward_impedance_xm_x = - impedance_norm_x
                if ((impedance_norm_y < 0.05) and (impedance_norm_y > 0.01)):
                    reward_impedance_xm_y = - impedance_norm_y
                if ((impedance_norm_phi < 0.05) and (impedance_norm_phi > 0.01)):
                    reward_impedance_xm_phi = - impedance_norm_phi
                #if(impedance_norm_phi < 0.01):
                #    reward_impedance_xm = + 10
                if (impedance_norm_x < 0.01):
                    reward_impedance_xm_x = + 15
                if (impedance_norm_y < 0.01):
                    reward_impedance_xm_y = + 15
                if (impedance_norm_phi < 0.01):
                    reward_impedance_xm_phi = + 10
                if (impedance_norm_x < 0.01 and impedance_norm_y < 0.01 and current_location1[1] >= 0.2):
                    reward_impedance_spring = + 30
                else:
                    reward_impedance_spring = 0

                #impedance_accuracy_vec = np.array(self.x0_equation).reshape(-1,1)-self.xm
                #impedance_accuracy_x = impedance_accuracy_vec[0]
                #impedance_accuracy_y = impedance_accuracy_vec[1]
                #impedance_accuracy_phi = impedance_accuracy_vec[2]

                #if(impedance_accuracy_x)>0.01:
                #    reward_accuracy_x = -2
                #if (impedance_accuracy_y) > 0.01:
                #    reward_accuracy_y = -2
                #if (impedance_accuracy_phi) > 0.01:
                #    reward_accuracy_phi = -2

                #if(impedance_accuracy_x)<0.01:
                #    reward_accuracy_x = +10
                #if (impedance_accuracy_y) < 0.01:
                #    reward_accuracy_y = +10
                #if (impedance_accuracy_phi) < 0.01:
                #    reward_accuracy_phi = +10
                ...
                #if((impedance_velocity_norm_xy > 0.2) or (impedance_velocity_norm_phi > 0.2)):
                if (impedance_velocity_norm_x > 0.02):
                    reward_impedance_xmDot_x = -5
                if (impedance_velocity_norm_y > 0.02):
                    reward_impedance_xmDot_y = -5
                if (impedance_velocity_norm_phi > 0.02):
                    reward_impedance_xmDot_phi = -5
                    """""
                    if self.sim.data.ncon > 0:
                        global self_hit_penalty
                        for i in range(self.sim.data.ncon):
                            contact = self.sim.data.contact[i]
                            if (contact.geom1 in [4, 5, 6, 7, 8] and contact.geom2 in [4, 5, 6, 7, 8]):
                                self_hit_penalty = True
                    else:
                        self_hit_penalty = False
                    if (self_hit_penalty):
                        reward_panalty = -10
                    if (not self_hit_penalty):
                        reward_panalty = 0
                    """""

                #if (((impedance_velocity_norm_xy < 0.2) and (impedance_velocity_norm_xy > 0.01)) or ((impedance_velocity_norm_phi < 0.2) and (impedance_velocity_norm_phi  > 0.01))):
                ...
                #    reward_impedance_xmDot = + 10
                #if (impedance_velocity_norm_x < 0.01):
                #    reward_impedance_xmDot_x = + 10
                #if (impedance_velocity_norm_y < 0.01):
                #    reward_impedance_xmDot_y = + 10
                #if (impedance_velocity_norm_phi < 0.01):
    ...
    def reset_model(self):
        ...
        #qpos = np.array([np.pi/2,-np.pi*3/4,np.pi*2/3,0,0,0,0])
        #qpos = np.array([np.pi*5/4, -np.pi*3/4, -np.pi *2/ 6, 0, 0, 0, 0])
        #qpos = np.array([-np.pi / 2, np.pi / 2,0, 0, 0, 0, 0])
        #fingertip=self.get_body_com("fingertip")
    ...
    def _get_obs(self):
        ...
        xm_temp = self.xm.flat[0:]
        xmDot_temp = self.xmDot.flat[0:]
        ...
        return np.concatenate([
            ...
            self.get_body_com("fingertip")[0:2] - xm_temp[0:2], # impedance state
            np.cos(np.array(self.sim.data.qpos[0] + self.sim.data.qpos[1] + self.sim.data.qpos[2]).flat[0:] - np.array(xm_temp[2]).flat[0:]),
            np.sin(np.array(self.sim.data.qpos[0] + self.sim.data.qpos[1] + self.sim.data.qpos[2]).flat[0:] - np.array(xm_temp[2]).flat[0:]),
            fingertip_vel[0:2]- xmDot_temp[0:2], # impedance velocity state
            np.array(fingertip_vel[2]).flat[0:] - np.array(xmDot_temp[2]).flat[0:],
