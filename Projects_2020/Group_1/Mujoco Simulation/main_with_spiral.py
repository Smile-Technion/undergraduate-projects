...
model = load_model_from_path("/home/yamitge/git_workspace/mj_sim/UR5_our/UR5gripper_box.xml")
sim = MjSim(model)
...
while True:
    ...
    if controller == 'PID':
        ...
        if pleg==1:
            ...
            Sircol[2, 3] = P0[2, 3]
            theta = theta + 0.01
            q_d, q_d_dot, x_im = impedance.imic(sim, q_r, q_r_dot, Sircol, 0, DT, force_log[2, -1], force_scope_log)  # force_log[2, -1]
            S = np.append(S, np.reshape(Sircol[0:3, 3], (3, 1)), axis=1)
            pleg = 1
